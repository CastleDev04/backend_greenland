...
function yn(t, e) {
  ...
  if (f = t.d, h = e.d, s = t.e, i = e.e, f = f.slice(), o = s - i, o) {
    for (o < 0 ? (n = f, o = -o, a = h.length) : (n = h, i = s, a = f.length), s = Math.ceil(C / U), a = s > a ? s + 1 : a + 1, o > a && (o = a, n.length = 1), n.reverse(); o--;) n.push(0);
    n.reverse();
  }
...
function hn(t, e) {
  ...
  for (e == null ? (N = !1, a = C) : a = e, s = new T(.03125); t.abs().gte(.1);) t = t.times(s), h += 5;
...
function it(t, e) {
  ...
  } else return f = vr(S, h + 2, I).times(o + ""), R = it(new S(n + "." + r.slice(1)), h - k).plus(f), S.precision = I, e == null ? (N = !0, _(R, I)) : R;
  for (a = s = R = he(R.minus(ee), R.plus(ee), h), T = _(R.times(R), h), i = 3;;) {
    if (s = _(s.times(T), h), f = a.plus(he(s, new S(i), h)), ue(f.d).slice(0, h) === ue(a.d).slice(0, h)) return a = a.times(2), o !== 0 && (a = a.plus(vr(S, h + 2, I).times(o + ""))), a = he(a, new S(C), h), S.precision = I, e == null ? (N = !0, _(a, I)) : a;
    a = f, i += 2;
  }
}
function dn(t, e) {
...
function _(t, e, r) {
  ...
  if (n = e - s, n < 0) n += U, i = e, h = C[T = 0];else {
    if (T = Math.ceil((n + 1) / U), o = C.length, T >= o) return t;
    for (h = o = C[T], s = 1; o >= 10; o /= 10) s++;
    n %= U, i = n - U + s;
  }
  if (r !== void 0 && (o = ke(10, s - i - 1), a = h / o % 10 | 0, f = e < 0 || C[T + 1] !== void 0 || h % o, f = r < 4 ? (a || f) && (r == 0 || r == (t.s < 0 ? 3 : 2)) : a > 5 || a == 5 && (r == 4 || f || r == 6 && (n > 0 ? i > 0 ? h / ke(10, s - i) : 0 : C[T - 1]) % 10 & 1 || r == (t.s < 0 ? 8 : 7))), e < 1 || !C[0]) return f ? (o = $(t), C.length = 1, e = e - o - 1, C[0] = ke(10, (U - e % U) % U), t.e = Ue(-e / U) || 0) : (C.length = 1, C[0] = t.e = t.s = 0), t;
  if (n == 0 ? (C.length = T, o = 1, T--) : (C.length = T + 1, o = ke(10, U - n), C[T] = i > 0 ? (h / ke(10, s - i) % ke(10, i) | 0) * o : 0), f) for (;;) if (T == 0) {
    (C[0] += o) == Q && (C[0] = 1, ++t.e);
    break;
  } else {
...
function bn(t, e) {
  ...
  if (f = t.d, C = e.d, n = e.e, h = t.e, f = f.slice(), s = h - n, s) {
    for (T = s < 0, T ? (r = f, s = -s, a = C.length) : (r = C, n = h, a = f.length), i = Math.max(Math.ceil(R / U), a) + 2, s > i && (s = i, r.length = 1), r.reverse(), i = s; i--;) r.push(0);
    r.reverse();
  } else {
...
function ds(t) {
  ...
  for (e = 0; e < i.length; e += 3) if ((n = t[r = i[e]]) !== void 0) if (Ue(n) === n && n >= i[e + 1] && n <= i[e + 2]) this[r] = n;else throw Error(Oe + r + ": " + n);
...
var Fe,
  ...
  En = ne(() => {
    ...
    }, N = !0, ie = "[DecimalError] ", Oe = ie + "Invalid argument: ", Tr = ie + "Exponent out of range: ", Ue = Math.floor, ke = Math.pow, ps = /^(\d+(\.\d*)?|\.\d+)(e[+-]?\d+)?$/i, Q = 1e7, U = 7, gn = 9007199254740991, Mt = Ue(gn / U), A = {};
    ...
    A.squareRoot = A.sqrt = function () {
      ...
      for (t = $(a), N = !1, i = Math.sqrt(+a), i == 0 || i == 1 / 0 ? (e = ue(a.d), (e.length + t) % 2 == 0 && (e += "0"), i = Math.sqrt(e), t = Ue((t + 1) / 2) - (t < 0 || t % 2), i == 1 / 0 ? e = "5e" + t : (e = i.toExponential(), e = e.slice(0, e.indexOf("e") + 1) + t), n = new f(e)) : n = new f(i.toString()), r = f.precision, i = s = r + 3;;) if (o = n, n = o.plus(he(a, o, s + 2)).times(.5), ue(o.d).slice(0, s) === (e = ue(n.d)).slice(0, s)) {
    ...
    A.toPower = A.pow = function (t) {
      ...
      if (e = t.e, r = t.d.length - 1, s = e >= r, o = a.s, s) {
        if ((r = T < 0 ? -T : T) <= gn) {
          for (i = new f(ee), e = Math.ceil(n / U + 4), N = !1; r % 2 && (i = i.times(a), fn(i.d, e)), r = Ue(r / 2), r !== 0;) a = a.times(a), fn(a.d, e);
          return N = !0, t.s < 0 ? new f(ee).div(i) : _(i, n);
        }
      } else if (o < 0) throw Error(ie + "NaN");
      return o = o < 0 && t.d[Math.max(e, r)] & 1 ? -1 : 1, a.s = 1, N = !1, i = t.times(it(a, n + h)), N = !0, i = hn(i), i.s = o, i;
    };
    ...
    he = function () {
      ...
      return function (n, i, o, s) {
        ...
        for (f = n.e - i.e, se = q.length, Se = le.length, R = new kt(zo), O = R.d = [], h = 0; q[h] == (le[h] || 0);) ++h;
        if (q[h] > (le[h] || 0) && --f, o == null ? H = o = kt.precision : s ? H = o + ($(n) - $(i)) + 1 : H = o, H < 0) return new kt(0);
        if (H = H / U + 2 | 0, h = 0, se == 1) for (T = 0, q = q[0], H++; (h < Se || T) && H--; h++) L = T * Q + (le[h] || 0), O[h] = L / q | 0, T = L % q | 0;else {
          for (T = Q / (q[0] + 1) | 0, T > 1 && (q = t(q, T), le = t(le, T), se = q.length, Se = le.length), z = se, S = le.slice(0, se), I = S.length; I < se;) S[I++] = 0;
          St = q.slice(), St.unshift(0), Er = q[0], q[1] >= Q / 2 && ++Er;
          do T = 0, a = e(q, S, se, I), a < 0 ? (oe = S[0], se != I && (oe = oe * Q + (S[1] || 0)), T = oe / Er | 0, T > 1 ? (T >= Q && (T = Q - 1), C = t(q, T), k = C.length, I = S.length, a = e(C, S, k, I), a == 1 && (T--, r(C, se < k ? St : q, k))) : (T == 0 && (a = T = 1), C = q.slice()), k = C.length, k < I && C.unshift(0), r(S, C, I), a == -1 && (I = S.length, a = e(q, S, se, I), a < 1 && (T++, r(S, se < I ? St : q, I))), I = S.length) : a === 0 && (T++, S = [0]), O[h++] = T, a && S[0] ? S[I++] = le[z] || 0 : (S = [le[z]], I = 1); while ((z++ < Se || S[0] !== void 0) && H--);
        }
...
It should have this form: { url: "CONNECTION_STRING" }`);
    ...
    adapter: (t, e) => {
      if (!t && qe(e.generator) === "client") throw new D('Using engine type "client" requires a driver adapter to be provided to PrismaClient constructor.');
      if (t === null) return;
      ...
      if (qe(e.generator) === "binary") throw new D('Cannot use a driver adapter with the "binary" Query Engine. Please use the "library" Query Engine.');
    },
    datasourceUrl: t => {
